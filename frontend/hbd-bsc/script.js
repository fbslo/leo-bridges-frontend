...
function wrap(){
	...
	if (!error){
		if(window.hive_keychain) {
			window.hive_keychain.requestTransfer(hiveAccountFrom, "bnb-hbd", parseFloat(amount).toFixed(3), addressTo, "HBD", () => {
				console.log('Request sent!', amount);
			}, false);
		} else {
			alert("Send "+parseFloat(amount).toFixed(3)+" HBD to @bnb-hbd with memo: " + addressTo)
		}
	}
}
